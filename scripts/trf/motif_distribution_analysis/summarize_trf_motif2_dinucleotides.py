#!/usr/bin/env python3
"""Summarize motif-length-2 TRF BED records by exact dinucleotide motif.
...
2, then counts the consensus motif in column 17. It writes:
...
BASES = "ACGT"
DINUCLEOTIDE_ORDER = tuple(first + second for first in BASES for second in BASES)
TOTAL_KEY = "__total__"
...
def read_motif2_records(
    bed_path: Path,
    motif_len_col: int,
    motif_col: int,
    skip_header: bool,
) -> Iterable[Motif2Record]:
...
def motif_sort_key(motif: str) -> tuple[int, str]:
    if motif in DINUCLEOTIDE_ORDER:
        return (DINUCLEOTIDE_ORDER.index(motif), motif)
    return (len(DINUCLEOTIDE_ORDER), motif)
...
def write_counts_summary(
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow(["group", "value", "total_motif2", "motif", "count", "fraction"])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Count motif-length-2 / dinucleotide records in TRF BED files."
    )
    ...
    parser.add_argument(
        ...
        "--out-prefix",
        required=True,
        help="Output prefix, e.g. results/trf_motif2_dinucleotides",
    )
    parser.add_argument(
        "--motif-len-col",
        type=int,
...
def main() -> int:
    ...
    for bed_file in bed_files:
        for record in read_motif2_records(
            bed_file,
    ...
    counts_path = prefix.with_suffix(".motif2_counts.summary.tsv")
    lengths_path = prefix.with_suffix(".motif2_repeat_length_distribution.tsv")
    ...
    print(f"Found {total_records} motif-length-2 records")
